...
static BOOL IsPartyMonImmuneToMove(struct BattleSystem *battleSys, struct BattleStruct *ctx,
                                    struct AI_sDamageCalc *mon, u16 moveno)
{
    u8 moveType = ctx->moveTbl[moveno].type;
    switch (mon->ability) {
        case ABILITY_FLASH_FIRE:
        case ABILITY_WELL_BAKED_BODY:
            if (moveType == TYPE_FIRE) return TRUE;
            break;
        case ABILITY_LIGHTNING_ROD:
        case ABILITY_VOLT_ABSORB:
        case ABILITY_MOTOR_DRIVE:
            if (moveType == TYPE_ELECTRIC) return TRUE;
            break;
        case ABILITY_WATER_ABSORB:
        case ABILITY_STORM_DRAIN:
        case ABILITY_DRY_SKIN:
            if (moveType == TYPE_WATER) return TRUE;
            break;
        case ABILITY_SAP_SIPPER:
            if (moveType == TYPE_GRASS) return TRUE;
            break;
        case ABILITY_LEVITATE:
        case ABILITY_EARTH_EATER:
            if (moveType == TYPE_GROUND) return TRUE;
            break;
        case ABILITY_BULLETPROOF:
            if (IsBallOrBombMove(moveno)) return TRUE;
            break;
        case ABILITY_SOUNDPROOF:
            if (IsMoveSoundBased(moveno)) return TRUE;
            break;
        case ABILITY_WIND_RIDER:
            if (IsMoveWindMove(moveno)) return TRUE;
            break;
        default:
            break;
    }
    ...
    u32 flag = 0;
    return BattleAI_GetTypeEffectiveness(battleSys, ctx, moveno, moveType, &flag,
                                          &dummyAttacker, mon) == TYPE_MUL_NO_EFFECT;
}
...
static BOOL TrainerAI_ImmunitySwitch(struct BattleSystem *battleSys, int battler)
{
    ...
    for (int k = 0; k < 4; k++) {
        ...
        if (moveno == MOVE_NONE) continue;
        struct BattleMove mv = ctx->moveTbl[moveno];
        if (mv.split == SPLIT_STATUS || !mv.power) continue;
        struct AI_damage damages = {0};
        damages.damageRoll = BattleAI_CalcDamage(battleSys, ctx, moveno,
            ctx->side_condition[BATTLER_IS_ENEMY(defender)], ctx->field_condition,
            mv.power, mv.type, 0, defender, battler,
            &damages, &playerMon, &aiMon);
    ...
    for (int k = 0; k < 4; k++) {
        ...
        struct AI_damage damages = {0};
        damages.damageRoll = BattleAI_CalcDamage(battleSys, ctx, moveno,
            ctx->side_condition[BATTLER_IS_ENEMY(battler)], ctx->field_condition,
            mv.power, mv.type, 0, battler, defender,
            &damages, &aiMon, &playerMon);
    ...
    for (int i = 0; i < partySize; i++) {
        ...
        if (GetMonData(mon, MON_DATA_HP, 0) == 0) continue;
        if (i == ctx->sel_mons_no[battler] || i == ctx->aiSwitchedPartySlot[battler]) continue;
        ...
        for (int k = 0; k < 4; k++) {
            ...
            damages.damageRoll = BattleAI_CalcDamage(battleSys, ctx, moveno,
                ctx->side_condition[BATTLER_IS_ENEMY(defender)], ctx->field_condition,
                mv.power, mv.type, 0, defender, battler,
                &damages, &playerMon, &partyMonData);
        ...
        BOOL qualifies25 = highestDmgMove != MOVE_NONE &&
                           IsPartyMonImmuneToMove(battleSys, ctx, &partyMonData, highestDmgMove);
        BOOL qualifies50 = playerLastMove != MOVE_NONE &&
                           IsPartyMonImmuneToMove(battleSys, ctx, &partyMonData, playerLastMove);

        if (!qualifies25 && !qualifies50) continue;
        ...
        for (int k = 0; k < 4; k++) {
            ...
            damages.damageRoll = BattleAI_CalcDamage(battleSys, ctx, moveno,
                ctx->side_condition[BATTLER_IS_ENEMY(battler)], ctx->field_condition,
                mv.power, mv.type, 0, battler, defender,
                &damages, &partyMonData, &playerMon);
            damages.damageRange[15] = BattleAI_AdjustUnusualMoveDamage(partyMonData.level, partyMonData.hp,
                playerMon.hp, damages.damageRange[15], mv.effect, partyMonData.ability, partyMonData.item);
        ...
        if (playerMon.hp == 0 || maxDmgDealt * 100 / playerMon.hp < 35) continue;
...
BOOL TrainerAI_ShouldSwitch(struct BattleSystem *battleSys, int battler)
{
    ...
    if ((ctx->battlemon[battler].effect_of_moves & MOVE_EFFECT_FLAG_PERISH_SONG_ACTIVE) &&
        ctx->battlemon[battler].moveeffect.perishSongTurns == 1) {
        int partySize = Battle_GetClientPartySize(battleSys, battler);
        for (int i = 0; i < partySize; i++) {
            if (i == ctx->sel_mons_no[battler] || i == ctx->aiSwitchedPartySlot[battler]) continue;
            struct PartyPokemon *mon = Battle_GetClientPartyMon(battleSys, battler, i);
